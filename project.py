...
queries = {
    1: {"title" : "Retrieve all orders based on cust",
        "sql" : """
                SELECT orderID, ROUND(totalAmmount, 2) AS totalAmmount
                FROM Orders
                WHERE custID = ?"""
        },
    2: {"title" : "List all dishes at restraunt",
        "sql" : """
                SELECT dishName, ROUND(dishPrice, 2) AS totalAmmount
                FROM Dish
                WHERE restrauntID = ?"""
        },
    3: {"title" : "Total spent by customer on all orders",
        "sql" : """
                SELECT Customers.custID, custFirstName, custSecondName, ROUND(SUM(totalAmmount), 2) AS totalSpent
                FROM Orders
    ...
    8: {"title" : "Average dish price per restraunt",
        "sql" : """
                SELECT Restraunt.restrauntID, restrauntName, ROUND(AVG(dishPrice), 2) AS avgDishPrice
                FROM Dish
                JOIN Restraunt ON Dish.restrauntID = Restraunt.restrauntID
    ...
    10: {"title" : "Total revenue generated by each restraunt",
        "sql" : """
                SELECT Restraunt.restrauntID, restrauntName, ROUND(SUM(totalAmmount), 2) AS totalRevenue
                FROM Orders
                JOIN Dish ON Orders.dishID = Dish.dishID
                JOIN Restraunt ON Dish.restrauntID = Restraunt.restrauntID
                GROUP BY Restraunt.restrauntID"""
    ...
    12: {"title" : "Each dish ordered with calculated total cost",
        "sql" : """
                SELECT orderID, dishName, quantity, ROUND(dishPrice,2), ROUND(dishPrice * quantity, 2) AS calculatedTotal
                FROM Orders
                JOIN Dish ON Orders.dishID = Dish.dishID"""
        },
    13: {"title" : "Orders placed within a date range",
        "sql" : """
                SELECT orderID, custID, orderDate, ROUND(totalAmmount, 2) AS totalAmmount
                FROM Orders
...
def add_new_order():
    ...
    total_amount = round((quantity * dish_price) + delivery_fee, 2)
